...
class PerspectiveCamera {
 public:
  PerspectiveCamera(const CameraSpec& spec) : center_(spec.center), direction_(glm::normalize(spec.direction)), up_(glm::normalize(spec.up)) {
  ...
  Ray GenerateRay(const glm::vec2& point) {
    ...
    glm::vec3 new_dir = d * direction_ + point[0] * horizontal_ + point[1] * up_;

    new_dir = glm::normalize(new_dir);

    return {center_, new_dir};
  }
